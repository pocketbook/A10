 ...
 */
#define DEBUG	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
...
//		ispctrl_val &= ~ISPCTRL_PAR_BRIDGE_BENDIAN;	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
       ispctrl_val |= ISPCTRL_PAR_BRIDGE_BENDIAN;		//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
...
//&*&*&*CJ2_20110811_mod: follow TI MMS' solution for camera driver
		    ...
		    }	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		}
	default:
		/*
...
//&*&*&*CJ1_20110811_mod: follow TI MMS' solution for camera driver
       ...
       //        __func__, ISP_BUFS_IS_EMPTY(bufs)==0, bufs->queue,  bufs->done);
		...
		if(CCDC_RESZ_CAPTURE(isp)) {
//&*&*&* TI_1 8/5 for resizer issue.			
//            if (isp_wait(dev, ispccdc_busy, 0, 5000, &isp->isp_ccdc)) {		//TI 8/5 for resizer issue.
...
//	int reset = 0;	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	int reset = 1;	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
...
//&*&*&*JJ1_20110624 for ISP camera
//#ifndef CONFIG_VIDEO_MT9D115	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	if (!reset)
		return;
//#endif			//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
//&*&*&*JJ2_20110624 for ISP camera
...
static void isp_set_buf(struct device *dev, struct isp_buf *buf)
{
	...
	if (isp->pipeline.modules == OMAP_ISP_CSIARX) {
		...
		isp_csi2_ctx_update(&isp->isp_csi2, 0, false);
//	} else if (CCDC_PREV_RESZ_CAPTURE(isp))					//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	} else if (CCDC_PREV_RESZ_CAPTURE(isp) || CCDC_RESZ_CAPTURE(isp))	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
...
//		if ((pix_output->width == 1280) &&		//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
//		    (pix_output->height == 720)) {		//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
			pipe->modules = OMAP_ISP_PREVIEW |
					OMAP_ISP_RESIZER |	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
					OMAP_ISP_CCDC;
			if (isp->revision <= ISP_REVISION_2_0) {	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
...
//		}	//&*&*&*CL2
		...
		if (pix_input->pixelformat == V4L2_PIX_FMT_SGRBG10 ||
			...
			pipe->modules = OMAP_ISP_RESIZER | OMAP_ISP_CCDC;	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
			pipe->ccdc_in = CCDC_YUV_SYNC;
//			pipe->ccdc_out = CCDC_OTHERS_MEM;			//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
			pipe->ccdc_out = CCDC_YUV_RSZ;				//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		} else
			return -EINVAL;
	}
...
#if 0	//&*&*&*CJ1_20110811_mod: follow TI MMS' solution for camera driver
...
#endif	//&*&*&*CJ2_20110811_mod: follow TI MMS' solution for camera driver
			  ...
			  enum isp_interface_type sensor_isp_if)
{
	...
	if (pipe.modules & OMAP_ISP_RESIZER) {
    	...
    	dumpIspPipeSettings(&pipe);		//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
...
printk("<%s>[%d]\n", __func__, __LINE__);		//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver		
		/* Tell ISP not to write any of our buffers. */
//		if (CCDC_CAPTURE(isp))			//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		if (CCDC_CAPTURE(isp) || CCDC_RESZ_CAPTURE(isp))	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
			isp_disable_interrupts(dev);
		if (isp->pipeline.modules == OMAP_ISP_CSIARX) {
...
//		a->value = isp->isp_csi2.force_mem_out ? 1 : 0;	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		a->value = isp->isp_csi2.force_mem_out ? 0 : 0;	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		break;
	default:
		rval = -EINVAL;
		break;
...
int isp_s_ctrl(struct device *dev, struct v4l2_control *a)
{
	...
	case V4L2_CID_PRIVATE_OMAP3ISP_CSI2MEM:
		/* NOTE: User must call again VIDIOC_S_FMT to make this
			 effective */
//		isp->isp_csi2.force_mem_out = new_value ? true : false;	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
        isp->isp_csi2.force_mem_out = new_value ? false : false;	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		break;
	default:
		rval = -EINVAL;
		break;
...
static int isp_probe(struct platform_device *pdev)
{
	...
	old_ick = clk_get_rate(isp->cam_ick);	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	...
	old_mclk = clk_get_rate(isp->cam_mclk);	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	...
	old_dpll4_m5_ck = clk_get_rate(isp->dpll4_m5_ck);	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	...
	old_csi2_96m_fck = clk_get_rate(isp->csi2_fck);		//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	...
	old_l3_ick = clk_get_rate(isp->l3_ick);	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
