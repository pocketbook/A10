...
#include <linux/mmc/host.h>	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
				       ...
				       int mask)
{
	...
	for (i = start; i != end + dir; i += dir) {
		if (vdev->slave[i] == v4l2_int_device_dummy())
			continue;
//&*&*&*CJ1_20110811_mod: follow TI MMS' solution for camera driver
		/* If is not power off, skip unused sensor. */
		if ((power != V4L2_POWER_OFF) && (i < 2) && (vdev->current_sensor != i)) continue;
//&*&*&*CJ2_20110811_mod: follow TI MMS' solution for camera driver
			   ...
			   struct v4l2_capability *cap)
{
	...
	struct omap34xxcam_fh *ofh = to_omap34xxcam_fh(vfh);
	struct omap34xxcam_videodev *vdev = ofh->vdev;
	struct v4l2_int_device *vsensor = (vdev->current_sensor == 0) ? vdev->vdev_sensor0 : vdev->vdev_sensor1;	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
...
//	if (vdev->vdev_sensor != v4l2_int_device_dummy())	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	if (vsensor != v4l2_int_device_dummy())			//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
...
//		*best_ival = vdev->vdev_sensor_config.ival_default;	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		*best_ival = vsensor_config->ival_default;		//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
...
////		rval = vidioc_int_enum_fmt_cap(vdev->vdev_sensor, &fmtd);	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		rval = vidioc_int_enum_fmt_cap(vsensor, &fmtd);			//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		if (rval)
			break;
...
//			rval = vidioc_int_enum_framesizes(vdev->vdev_sensor,	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
			rval = vidioc_int_enum_framesizes(vsensor,		//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
							  &frms);
			if (rval)
				break;
...
//					       vdev->vdev_sensor_config.isp_if);	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
					       vsensor_config->isp_if);			//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
			if (rval)
				return rval;
...
//					vdev->vdev_sensor, &frmi);	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
					vsensor, &frmi);		//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
				if (rval)
					break;
...
//	rval = vidioc_int_s_parm(vdev->vdev_sensor, &a);	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	rval = vidioc_int_s_parm(vsensor, &a);			//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver

//	vidioc_int_priv_g_pixclk_active(vdev->vdev_sensor, &pixclk);	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	vidioc_int_priv_g_pixclk_active(vsensor, &pixclk);		///&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
...
{
//	if (inp->index > 0)				//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	if ((inp->index < 0) || (inp->index > 1))	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		return -EINVAL;
			 ...
			 struct v4l2_streamparm *a)
{
	...
	mutex_lock(&vdev->mutex);
//	rval = vidioc_int_g_parm(vdev->vdev_sensor, a);	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	rval = vidioc_int_g_parm(vsensor, a);		//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	mutex_unlock(&vdev->mutex);
...
//	rval = vidioc_int_cropcap(vdev->vdev_sensor, a);	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	rval = vidioc_int_cropcap(vsensor, a);			//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver

//	if (rval && !vdev->vdev_sensor_config.sensor_isp) {	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	if (rval && !vsensor_config->sensor_isp) {		//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		struct v4l2_format f;
		struct v4l2_rect pixel_size;

		/* cropcap failed, try to do this via g_fmt_cap */
//		rval = vidioc_int_g_fmt_cap(vdev->vdev_sensor, &f);	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		rval = vidioc_int_g_fmt_cap(vsensor, &f);		//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		if (!rval) {
			isp_g_bounds(isp, &cropcap->bounds);
			cropcap->defrect = cropcap->bounds;
		}

//		rval = vidioc_int_priv_g_pixelsize(vdev->vdev_sensor,	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		rval = vidioc_int_priv_g_pixelsize(vsensor,		//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
						   &pixel_size);
		if (!rval) {
...
//	rval = vidioc_int_enum_framesizes(vdev->vdev_sensor, frms);	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	rval = vidioc_int_enum_framesizes(vsensor, frms);		//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
...
//		rval = vidioc_int_enum_framesizes(vdev->vdev_sensor,	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	...
	frmi->width = width;
	frmi->height = height;
//	rval = vidioc_int_enum_frameintervals(vdev->vdev_sensor, frmi);	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	rval = vidioc_int_enum_frameintervals(vsensor, frmi);		//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
...
//&*&*&*CJ2_20110811_mod: follow TI MMS' solution for camera driver
	...
	if (cmd == VIDIOC_PRIVATE_OMAP34XXCAM_SENSOR_INFO) {
		...
		mutex_lock(&vdev->mutex);
//		rval = vidioc_int_priv_g_pixclk(vdev->vdev_sensor, &pixclk);	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		rval = vidioc_int_priv_g_pixclk(vsensor, &pixclk);		//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		mutex_unlock(&vdev->mutex);
		if (rval)
			goto out;
		mutex_lock(&vdev->mutex);
//		rval = vidioc_int_priv_g_activesize(vdev->vdev_sensor,		//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		rval = vidioc_int_priv_g_activesize(vsensor,			//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
					    &active_size);
		mutex_unlock(&vdev->mutex);
		if (rval)
			goto out;
		mutex_lock(&vdev->mutex);
//		rval = vidioc_int_priv_g_fullsize(vdev->vdev_sensor,		//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		rval = vidioc_int_priv_g_fullsize(vsensor,			//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
						  &full_size);
		mutex_unlock(&vdev->mutex);
		if (rval)
			goto out;
...
//	if (vdev->vdev_sensor_config.sensor_isp) {				//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	if (vsensor_config->sensor_isp) {					//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		rval = -EINVAL;
	} else {
		switch (cmd) {
		case VIDIOC_PRIVATE_ISP_AEWB_REQ:
		{
			...
			if (data->update & SET_EXPOSURE) {
				...
				mutex_lock(&vdev->mutex);
//				rval = vidioc_int_s_ctrl(vdev->vdev_sensor,	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
				rval = vidioc_int_s_ctrl(vsensor,		//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
							 &vc);
				mutex_unlock(&vdev->mutex);
				if (rval)
					goto out;
...
static int omap34xxcam_open(struct file *file)
{
	...
	struct v4l2_int_device *vsensor;	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	int first_user = 0;
	int i;
...
//	}								//&*&*&*CJ_20110610 for improve captrure performance
	...
	vsensor = (vdev->current_sensor == 0) ? vdev->vdev_sensor0 : vdev->vdev_sensor1;	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver

//	if (vdev->vdev_sensor == v4l2_int_device_dummy() || !first_user)	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	if (vsensor == v4l2_int_device_dummy() || !first_user)			//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		goto out_no_pix;

	/* Get the format the sensor is using. */
//	rval = vidioc_int_g_fmt_cap(vdev->vdev_sensor, &sensor_format);		//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	rval = vidioc_int_g_fmt_cap(vsensor, &sensor_format);			//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	if (rval) {
...
//	if (hwc.dev_type == OMAP34XXCAM_SLAVE_SENSOR) {		//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	if (hwc.dev_type == OMAP34XXCAM_SLAVE_SENSOR0) {	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		struct v4l2_format format;

		format.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
//		rval = vidioc_int_g_fmt_cap(vdev->vdev_sensor, &format);	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		rval = vidioc_int_g_fmt_cap(vdev->vdev_sensor0, &format);	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		if (rval)
			rval = -EBUSY;
...
static int __init omap34xxcam_init(void)
{
	...
	for (i = 0; i < OMAP34XXCAM_VIDEODEVS; i++) {
		...
		vdev->index             = i;
		vdev->current_sensor	= 0;	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
		vdev->cam               = cam;
//&*&*&*CJ1_20110811_mod: follow TI MMS' solution for camera driver
//		vdev->vdev_sensor =
		vdev->vdev_sensor0 =
