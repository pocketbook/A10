...
#define CHARGING_FULL_VCELL 412000
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(val == 0)
	{
		lsb = lsb | (MAX_LOW_BATTERY_THRESHOLD - (chip->low_battery*2+2));
		value = (msb<<8) | lsb;
	}
	else
	{
		lsb = lsb | (MAX_LOW_BATTERY_THRESHOLD - (val*2+2));
		value = lsb;
	}
...
{
	/* 
	*   It is a software patch to make the part work with higher impedance batteries.  
	*   Since the SOC reading is changed to have 0.5% LSB for 2^0, 
	*   the alert also has to have a LSB change for 0.5% for 2^0.
	*   The alert in this case has a maximum range of 16% instead of 32%, 
	*   so you have to set it to 20% to trigger at 10%.
	*/
	int config_value = max17043_read_reg(client, MAX17040_RCOMP_MSB);

	config_value = (config_value << 8) | (MAX_LOW_BATTERY_THRESHOLD-(value*2+2));
...
static void max17043_work(struct work_struct *work)
{
	...
	*/
	...
	printk("[Max17043]VCELL %d, Capacity %d, On-line %d\n", chip->vcell, chip->soc, chip->online);
#endif
...
#ifdef CONFIG_REGULATOR_MAX8903
	if (chip->pdata->battery_is_charging)
		chip->charging_status = chip->pdata->battery_is_charging();
	if(chip->soc == 100)
	{
	...
	else if((chip->soc <= 99) && (!chip->charging_status && !chip->online))
	{
		if(chip->vcell > CHARGING_FULL_VCELL)
		{
			...
			printk("[max17043 gauge] Enter %s, maybe temperature fault.\n", __FUNCTION__);
	}
#endif
	/* 
