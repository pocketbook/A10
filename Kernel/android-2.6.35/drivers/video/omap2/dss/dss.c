...
int dss_set_clock_div(struct dss_clock_info *cinfo)
{
	...
	if (cpu_is_omap34xx()) {
		prate = clk_get_rate(clk_get_parent(dss.dpll4_m4_ck));
		printk("[Jimmy]dpll4_m4 = %ld, cinfo->fck_div=%d \n", prate,cinfo->fck_div);
...
printk("[Jimmy]min_fck_per_pck=%d, req_pck=%d, min_fck_per_pck=%d, DISPC_MAX_FCK=%d \n",
							min_fck_per_pck, req_pck, min_fck_per_pck, DISPC_MAX_FCK);
