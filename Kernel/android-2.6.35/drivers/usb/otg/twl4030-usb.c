...
int twl4030_accessory_enable(int enable)
{
	printk("[twl4030-usb]Enter %s, enable:%d.\n", __FUNCTION__, enable);
	if(enable)
	{
...
EXPORT_SYMBOL(twl4030_accessory_enable);
#endif
...
//&*&*&*AL1_20110816: read value of dp_dm for charger detection
int twl4030_cable_DP_DM_status(void)
{
	int val = 0;
	val = twl4030_usb_read(g_twl_usb, USB_DEBUG);
	printk("[twl4030-usb]USB_DEBUG=0x%02x \n", val);
	return val;
}
EXPORT_SYMBOL(twl4030_cable_DP_DM_status);
//&*&*&*AL1_20110816: read value of dp_dm for charger detection
