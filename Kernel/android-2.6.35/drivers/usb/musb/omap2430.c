		...
		unsigned long event, void *unused)
{
	...
	case USB_EVENT_ID:
		...
		//&*&*&*QY_20110706, fix low-speed otg
		
		omap_set_vbus(musb,1);//simon
...
//&*&*&*SJ2_20110706, merge #change,14303
//&*&*&*QY_20110706, fix low-speed otg
      printk("[msub]Unlock musb_wake_lock \n");
      wake_unlock(&pdata->musb_lock); //Simon
//&*&*&*QY_20110706      
		break;
	default:
		printk("ID float\n");
...
//&*&*&*QY_20110706, fix low-speed otg
extern int twl4030_otg_enable(int enable);
//&*&*&*QY_20110706 
static void omap_set_vbus(struct musb *musb, int is_on)
{
	u8		devctl;
	...
	if (is_on) {
		...
		MUSB_HST_MODE(musb);
		printk("omap_set_vbus OTG_STATE_A_WAIT_VRISE \n");		 
	} else {
		//&*&*&*QY_20110706, fix low-speed otg
		...
		struct musb_context_registers *musb_context)
{
	...
	musb_context->otg_forcestandby = musb_readl(musb->mregs,
							OTG_FORCESTDBY);	//&*&*&*SJ_20110706, merge #change,14303
...
{
	void __iomem *musb_base = musb->mregs;
	u32 forcestandby = musb_context->otg_forcestandby;	//&*&*&*SJ_20110706, merge #change,14303
