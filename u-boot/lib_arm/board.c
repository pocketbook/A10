...
//&*&*&*SJ1_20110620
{
	...
	Debug = result_env ? simple_strtoul (result_env, NULL, 6) : 0;
	...
	switch(boot_device) {
		...
		case BOOTING_TYPE_eMMC: /** eMMC  (MMC2) **/
			...
			if ((INITRAMFS_env == 1) && (g_BoardID < BOARD_ID_EVT3)) {
				...
				break;
			} else if ((INITRAMFS_env == 1) && (g_BoardID >= BOARD_ID_EVT3)) {
				break;
			}
#if defined (CONFIG_GPS_DEBUG_UART_SWITCH)
			if (g_BoardID < BOARD_ID_EVT3) { //&*&*&*SJ_20110607, add check HW id.
				if (Debug) setenv("bootargs", CONFIG_BOOTARGS_TFT_EMMC_BOOT);
				else       setenv("bootargs", CONFIG_BOOTARGS_TFT_EMMC_GPS_UART_BOOT);
			} else {
				setenv("bootargs", CONFIG_BOOTARGS_TFT_EMMC_BOOT);	
			}
#else
			setenv("bootargs", CONFIG_BOOTARGS_TFT_EMMC_BOOT);
#endif
