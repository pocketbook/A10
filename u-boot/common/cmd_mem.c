...
//&*&*&*SJ1_20110901, add "ddr_mtest" command.
/*
 * Perform a memory test. A more complete alternative test can be
 * configured using CFG_ALT_MEMTEST. The complete test loops until
 * interrupted by ctrl-c or by a failure of one of the sub-tests.
 */
int do_ddr_mem_mtest (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	vu_long	*addr, *start, *end;
	ulong	val;
	ulong	pattern;
	...
	start = (ulong *)CFG_MEMTEST_START;
	end = (ulong *)(CFG_MEMTEST_START+0x2000000);
	pattern = 0x5A5A5A5A5;
	for (;;) {
		printf ("\rDDR Test Write....");
		for (addr=start,val=pattern; addr<end; addr++) {
			*addr = val++;
			val = ~val + 0x2;
		}
		printf ("\rDDR Test Read....");
		for (addr=start,val=pattern; addr<end; addr++) {
			val = *addr;
			val = 0;
		}
...
//&*&*&*SJ2_20110901, add "ddr_mtest" command.

//&*&*&*20101201_Peter ++
int do_mem_mclear (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	vu_long *pMem;
	vu_long	addr, length;
	int i;
...
//&*&*&*SJ1_20110901, add "ddr_mtest" command.
U_BOOT_CMD(
	ddr_mtest,    4,    1,     do_ddr_mem_mtest,
	"ddr_mtest   - simple DDR RAM test\n",
	""
	""
);
//&*&*&*SJ2_20110901, add "ddr_mtest" command.
